...
from neuropixel_predictor.utils import fetch_receptive_fields
...
def plot_all_methods_per_neuron(
    ...
    receptive_fields = {dk: fetch_receptive_fields(dk) for dk in data_keys}
    ...
    for dk in data_keys:
        ...
        for n in range(N):
            ...
            # 4️⃣ RF SN Light plot
            plot_receptive_field(
                receptive_fields[dk]['rfs_sl'],
                cluster_id=cluster_ids[dk][n],
                title=f"Receptive Field - SparseNoise Light - Neuron: {n} (#{cluster_ids[dk][n]})",
                ax=axes['D']
            )

            # 5️⃣ RF SN Dark plot
            plot_receptive_field(
                receptive_fields[dk]['rfs_sd'],
                cluster_id=cluster_ids[dk][n],
                title=f"Receptive Field - SparseNoise Dark - Neuron: {n} (#{cluster_ids[dk][n]})",
                ax=axes['E']
            )

            fig.suptitle(
                f"({run_name}) - Neuron {n} (#{cluster_ids[dk][n]}) | Data key: {dk} | val_corrcoef: {test_neuron_stats[dk][n]:.4f}",
                fontsize=20,
            )
...
def plot_receptive_field(rfs, cluster_id, ax, title=''):
    rf = rfs[cluster_id]
